...
/* --- Proje ekibi ---
   Internal Audit ve Risk her projede zorunlu üyedir (CHANGELOG 1.13.1). Bu kodda ayrı bir
   "risk" rolü henüz yok; en yakın karşılıklar kullanılır: Internal Audit -> role_key='inspection'
   (Teftiş), Risk -> role_key='control' (İç Kontrol / Kurumsal Risk). Kurum ileride ayrı bir
   "risk" rolü tanımlarsa buradaki ROLE_KEY_FOR eşlemesi güncellenmelidir.
   documents.js'teki inspector() ile aynı desen: en az bir aktif kullanıcı bulunmalı,
   yoksa 409 döner — proje sessizce eksik ekiple kalmaz. */
const MANDATORY_ROLE_KEY_FOR = { "Internal Audit": "inspection", "Risk": "control" };

async function mandatoryAssignee(projectRole) {
  const roleKey = MANDATORY_ROLE_KEY_FOR[projectRole];
  const u = await db.one(
    "SELECT username FROM users WHERE role_key = $1 AND active ORDER BY username LIMIT 1", [roleKey]);
  if (!u) throw Object.assign(
    new Error(`${projectRole} için zorunlu ekip üyesi atanamadı: bu rolde aktif kullanıcı yok`),
    { status: 409 });
  return u.username;
}

/* Yeni projede ve mevcut projelerde (lazy backfill) zorunlu üyeliği tamamlar.
   Idempotent'tir: zaten üye olan kişi için ON CONFLICT DO NOTHING ile atlanır. */
async function ensureMandatoryMembers(projectId) {
  for (const projectRole of Object.keys(MANDATORY_ROLE_KEY_FOR)) {
    const already = await db.one(
      "SELECT username FROM project_members WHERE project_id=$1 AND project_role=$2",
      [projectId, projectRole]);
    if (already) continue;
    const username = await mandatoryAssignee(projectRole);
    await db.query(
      `INSERT INTO project_members (project_id, username, project_role, is_mandatory)
       VALUES ($1,$2,$3,TRUE) ON CONFLICT (project_id, username) DO NOTHING`,
      [projectId, username, projectRole]);
  }
}

async function teamOf(projectId) {
  return db.many(
    `SELECT m.username, u.display_name, m.project_role, m.is_mandatory, m.added_at
       FROM project_members m JOIN users u ON u.username = m.username
      WHERE m.project_id = $1
      ORDER BY m.is_mandatory DESC, m.project_role, u.display_name`, [projectId]);
}

module.exports = {
  EXEC_TITLES, GATE_AUTHORITY_TITLES, isExec, isGateAuthority, projectMetrics, burndown,
  velocity, distribution, executiveSummary, snapshotBurndown, DONE,
  ensureMandatoryMembers, teamOf,
};
