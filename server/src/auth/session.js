...
async function createSession(username, forcePasswordChange) {
  const id = randomId();
  ...
  await query(
    `INSERT INTO sessions (id, username, csrf_secret, idle_expires_at, absolute_expires_at, force_password_change)
     VALUES ($1,$2,$3,$4,$5,$6)`,
    [id, username, csrfSecret, idleExpiresAt, absoluteExpiresAt, !!forcePasswordChange]
  );
  return { id, csrfSecret, idleExpiresAt, absoluteExpiresAt };
}

async function completePasswordChange(id) {
  await query("UPDATE sessions SET force_password_change=false WHERE id=$1", [id]);
}
...
module.exports = { createSession, getSession, destroySession, destroyAllSessionsForUser, completePasswordChange };
